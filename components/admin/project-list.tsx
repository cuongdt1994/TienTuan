...
function SortableRow({ project }: { project: Project }) {
  ...
  return <tr ref={setNodeRef} style={{ transform: CSS.Transform.toString(transform), transition }} className="border-b border-line bg-paper"><td className="min-w-[300px] py-4 pr-6"><div className="flex items-center gap-3"><button {...attributes} {...listeners} className="cursor-grab touch-none text-muted" aria-label={`Reorder ${project.title}`}><GripVertical size={15} /></button>{project.coverImage ? <Image src={browserImageUrl(project.coverImage.thumbnailUrl)} alt="" width={56} height={56} unoptimized className="h-14 w-14 object-cover" style={{ objectPosition: `${project.coverPositionX}% ${project.coverPositionY}%` }} /> : <div className="h-14 w-14 bg-fog" />}<div><p className="whitespace-nowrap">{project.title}</p><p className="mt-1 text-xs text-muted">/{project.slug}</p></div></div></td><td className="min-w-[150px] whitespace-nowrap py-4 pr-6 text-muted">{project.category.name}</td><td className="min-w-[80px] whitespace-nowrap py-4 pr-6 text-muted">{project._count.images}</td><td className="min-w-[120px] whitespace-nowrap py-4 pr-6"><span className={`text-[10px] uppercase tracking-editorial ${project.status === "PUBLISHED" ? "text-emerald-700" : "text-muted"}`}>{project.status}</span></td><td className="min-w-[110px] whitespace-nowrap py-4"><div className="flex items-center gap-4"><Link href={`/admin/projects/${project.id}`} className="text-[10px] uppercase tracking-editorial underline underline-offset-4">Edit</Link>{project.status !== "PUBLISHED" && <PreviewLink projectId={project.id} />}</div></td></tr>;
}
    ...
    {groups.map(({ category, projects: categoryProjects }) => <section key={category.id}>
      ...
      <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={(event) => onDragEnd(category.id, event)}>
        <SortableContext items={categoryProjects.map((project) => project.id)} strategy={verticalListSortingStrategy}>
          <table className="w-full min-w-[820px] table-fixed text-left text-sm"><thead className="border-b border-line text-[10px] uppercase tracking-editorial text-muted"><tr><th className="w-[38%] pb-4 font-normal">Project</th><th className="w-[20%] pb-4 font-normal">Category</th><th className="w-[10%] pb-4 font-normal">Images</th><th className="w-[16%] pb-4 font-normal">Status</th><th className="w-[16%] pb-4 font-normal">Action</th></tr></thead><tbody>{categoryProjects.map((project) => <SortableRow key={project.id} project={project} />)}</tbody></table>
        </SortableContext>
      </DndContext>
    </section>)}
  </div>;
