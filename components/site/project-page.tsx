...
export async function getProjectPageMetadata(slug: string, previewToken?: string): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      ...
      type: "article",
      url: `/${project.slug}`,
      images: image ? [{ url: browserImageUrl(image.largeUrl), alt: image.alt ?? `${project.title} cover` }] : undefined,
    },
    twitter: { card: "summary_large_image", title: project.title, description: project.description ?? undefined },
  };
}
