...
export default async function AdminDashboardPage() {
  ...
  try {
    const [projects, published, images, categories] = await Promise.all([db.project.count(), db.project.count({ where: { status: "PUBLISHED" } }), db.image.count({ where: { deletedAt: null } }), db.category.count()]);
