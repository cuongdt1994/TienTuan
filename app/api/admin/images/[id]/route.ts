...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (body.cover) await db.project.update({ where: { id: image.projectId }, data: { coverImageId: image.id } });
  const updated = await db.image.update({ where: { id }, data: { ...(typeof body.alt === "string" ? { alt: body.alt } : {}) } });
