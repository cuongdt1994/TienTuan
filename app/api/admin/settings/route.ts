...
export async function PUT(request: Request) { if (!await requireAdmin()) return NextResponse.json({ error: "Unauthorized" }, { status: 401 }); const parsed = settingsSchema.safeParse(await request.json()); if (!parsed.success) return NextResponse.json({ error: "Invalid settings" }, { status: 400 }); const settings = await db.siteSettings.upsert({ where: { id: "site" }, update: parsed.data, create: { id: "site", ...parsed.data } }); return NextResponse.json({ settings }); }
